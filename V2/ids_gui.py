...
class IDSApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Soft context: per-patient message timestamps for repeat detection
        # This does NOT block messages — it adds context to events
        self._patient_msg_times  = defaultdict(list)   # {pid: [ts, ...]}
        self._ip_msg_times       = defaultdict(list)   # {ip: [ts, ...]}
    ...
    def _on_mqtt_message(self, client, userdata, msg):
        ...
        topic_hit = _hit_in_string(msg.topic)
        if topic_hit:
            event = {
        ...
        # Soft context: check for behavioural patterns that supplement
        # the model score without gating or overriding it
        context_flags = self._compute_context(source_ip,
                                               str(payload.get('patient_id', '')))

        # If model says CLEAN but context shows suspicious volume,
        # escalate to LOW so it appears in the table (not suppressed)
        if alert_level == 'CLEAN' and context_flags.get('burst_count', 0) > 30:
            alert_level = 'LOW'
            attack = {'type': 'STATISTICAL_ANOMALY',
                      'reason': 'High message volume from this IP ('
                                + str(context_flags['burst_count'])
                                + ' msgs/10s). Vitals score as normal '
                                'but volume pattern is unusual.',
                      'source': 'CONTEXT'}
    ...
    def _compute_context(self, source_ip: str, pid: str) -> dict:
        """Non-blocking context tracker. Returns behavioural signals.
        Does NOT block or gate messages — purely informational."""
        now = time.time()
        window = 10.0
        with self._context_lock:
            # Track IP message rate
            ip_ts = self._ip_msg_times[source_ip]
            ip_ts.append(now)
            self._ip_msg_times[source_ip] = [t for t in ip_ts if now - t <= window]
            burst_count = len(self._ip_msg_times[source_ip])

            # Track per-patient repeat rate
            pt_ts = self._patient_msg_times[pid]
            pt_ts.append(now)
            self._patient_msg_times[pid] = [t for t in pt_ts if now - t <= 3.0]
            patient_repeat = len(self._patient_msg_times[pid])

        return {'burst_count': burst_count, 'patient_repeat': patient_repeat}
