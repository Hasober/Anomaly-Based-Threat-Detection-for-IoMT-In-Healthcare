...
class RuleEngine:
    ...
    #
    # Legitimate pattern: 20 patients published every 3 seconds in a burst
    # from ONE trusted IP.  A real brute-force looks different:
    #   • Many msgs for the SAME patient in a short window  (targeted flood)
    #   • Far more than 20 unique patients from one IP      (impossible normally)
    #   • Sustained high volume over a longer window        (not a 3s burst)
    #
    # IP-level: allow up to 25 msgs / 3s  (handles 20-patient burst + slack)
    #           but flag > 50 msgs / 10s  (sustained flood that goes beyond normal)
    # Per-patient: allow up to 3 msgs / 3s per patient (one cycle + small lag)
    #              flag > 5 msgs / 3s per patient       (targeted repeat flood)

    IP_BURST_LIMIT      = 25    # max messages per IP in BURST_WINDOW seconds
    IP_BURST_WINDOW     = 3.0   # seconds — matches publisher cycle

    IP_SUSTAINED_LIMIT  = 60    # max messages per IP in SUSTAINED_WINDOW seconds
    IP_SUSTAINED_WINDOW = 10.0  # seconds — catches slow floods that slip burst check

    PATIENT_LIMIT       = 5     # max messages for ONE patient_id in PATIENT_WINDOW
    PATIENT_WINDOW      = 3.0   # seconds

    # Unique-patient diversity check: legitimate IP publishes many DIFFERENT patients.
    # Flag if same IP sends > REPEAT_THRESHOLD msgs for the SAME patient
    # within BURST_WINDOW (distinguishes scatter flood from single-patient flood).
    REPEAT_THRESHOLD    = 3

    def __init__(self):
        # {ip: [timestamp, ...]}  — all messages from this IP
        self._ip_timestamps      = defaultdict(list)
        # {ip: {patient_id: [timestamp, ...]}}  — per-IP per-patient tracking
        self._ip_patient_ts      = defaultdict(lambda: defaultdict(list))
    ...
    def check(self, payload: dict, topic: str, source_ip: str) -> dict | None:
        ...
        # ── Rule 7: Smart rate limiting ───────────────────────────────────────
        #
        # Three independent checks with context-aware thresholds:
        #   A) IP burst check    — more than 25 different msgs in 3s
        #   B) IP sustained check— more than 60 msgs in 10s (slow flood)
        #   C) Per-patient check — same patient_id > 5 times in 3s from same IP
        #
        # Legitimate publisher: 20 patients × 1 msg every 3s = 20 msgs / 3s
        # → passes A (20 < 25), passes B (20 < 60), passes C (1 per patient)
        #
        # Brute-force flood: 30 msgs for P003 in <1s
        # → passes A (maybe), passes B (maybe), FAILS C (>>5 for P003)
        #
        # Scatter flood: 60 msgs rotated across all patients in 3s
        # → FAILS A (60 > 25) and FAILS B

        with self._lock:
            # ── A) IP-level burst check ───────────────────────────────────────
            ip_ts = self._ip_timestamps[source_ip]
            ip_ts.append(now)
            # Prune to burst window
            burst_ts = [t for t in ip_ts if now - t <= self.IP_BURST_WINDOW]
            self._ip_timestamps[source_ip] = [
                t for t in ip_ts if now - t <= self.IP_SUSTAINED_WINDOW]

            if len(burst_ts) > self.IP_BURST_LIMIT:
                return self._verdict("HIGH", "BRUTE_FORCE",
                    f"IP {source_ip} sent {len(burst_ts)} messages in "
                    f"{self.IP_BURST_WINDOW}s (burst limit: {self.IP_BURST_LIMIT}). "
                    f"Normal publisher sends 20 msgs/cycle — this exceeds safe threshold.")

            # ── B) IP-level sustained flood check ────────────────────────────
            sustained_ts = self._ip_timestamps[source_ip]
            if len(sustained_ts) > self.IP_SUSTAINED_LIMIT:
                return self._verdict("HIGH", "BRUTE_FORCE",
                    f"IP {source_ip} sent {len(sustained_ts)} messages in "
                    f"{self.IP_SUSTAINED_WINDOW}s (sustained limit: {self.IP_SUSTAINED_LIMIT}). "
                    f"Possible slow flood attack.")

            # ── C) Per-patient repeat check from same IP ──────────────────────
            # Tracks how many times the SAME patient was published by SAME IP
            # within the burst window — legitimate: once per cycle
            pt_ts = self._ip_patient_ts[source_ip][pid]
            pt_ts.append(now)
            self._ip_patient_ts[source_ip][pid] = [
                t for t in pt_ts if now - t <= self.PATIENT_WINDOW]
            patient_count = len(self._ip_patient_ts[source_ip][pid])

            if patient_count > self.PATIENT_LIMIT:
                return self._verdict("HIGH", "BRUTE_FORCE",
                    f"Patient {pid} published {patient_count} times in "
                    f"{self.PATIENT_WINDOW}s by IP {source_ip} "
                    f"(limit: {self.PATIENT_LIMIT}). "
                    f"Possible targeted patient flood or replay loop.")
